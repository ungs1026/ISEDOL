...
let currentPlaylist = []; // 현재 플레이 중인 곡의 리스트
let shufflePlaylist = []; // 셔플 재생을 위한 곡 리스트
let tempPlaylist = []; // 임시로 저장할 플레이리스트
let audioElement; // 오디오 객체를 담을 변수
let mouseDown = false; // 마우스 클릭 상태를 저장하는 변수
let currentIndex = 0; // 현재 재생 중인 곡의 인덱스
let repeat = false; // 반복 재생 상태 변수
let shuffle = false; // 셔플 재생 상태 변수
let userId; // 사용자 ID 저장 변수
let timer; // 페이지 전환 시 타이머 변수
let seconds = 0;
...
function formatTime(seconds) {
  let time = Math.round(seconds); // 시간을 반올림
  let minutes = Math.floor(time / 60); // 분 계산
  seconds = time - (minutes * 60); // 초 계산
  let extraZero = (seconds < 10) ? "0" : ""; // 초가 10보다 작으면 앞에 0 추가
...
function updateTimeProgressBar(audio) {
  ...
  let progress = audio.currentTime / audio.duration * 100; // 진행 바의 퍼센트 계산
  $(".playbackBar .progress").css("width", progress + "%"); // 진행 바의 너비 업데이트
}

// 볼륨 진행 바의 퍼센트를 업데이트하는 함수
function updateVolumeProgressBar(audio) {
  let volume = audio.volume * 100; // 볼륨을 퍼센트로 변환
...
function Audio() {
  ...
  this.audio.addEventListener("canplay", function () {
    let duration = formatTime(this.duration); // 총 재생 시간 포맷
